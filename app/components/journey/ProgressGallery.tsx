...
const Shot = ({ shot, index, count }: { shot: ProgressShot, index: number, count: number }) => {
  ...
  const layout = useMemo(() => {
    ...
    const width = Math.min(HEIGHT * shot.aspect, isMobile ? 2.1 : 3.2);
    const height = width / shot.aspect;
    return {
      y: TOP + (BOTTOM - TOP) * (index / Math.max(1, count - 1)),
      x: side * (isMobile ? 0.55 + width * 0.2 : 1.1 + width * 0.3),
      z: 5 + (index % 3 - 1) * 0.35,
      tilt: side * 0.05,
      lean: side * -0.18,
      width,
  ...
  useFrame(({ camera, clock }) => {
    ...
    const read = THREE.MathUtils.smoothstep(above, 0.8, 2.4) * (1 - THREE.MathUtils.smoothstep(above, 5.5, 8));
